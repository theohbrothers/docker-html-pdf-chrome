...
``````sh
docker run --rm -it -e URL=http://localhost -e PDF_FILE=export.pdf -v `$(pwd):`$(pwd) -w `$(pwd) --network host theohbrothers/docker-html-pdf-chrome:$( $VARIANTS | % { $_['tag'] } | Select-Object -First 1 )
...
``````sh
docker run --rm -it -e SLEEP=1 -v `$(pwd):`$(pwd) -w `$(pwd) theohbrothers/docker-html-pdf-chrome:$( $VARIANTS | % { $_['tag'] } | Select-Object -First 1 )
``````
